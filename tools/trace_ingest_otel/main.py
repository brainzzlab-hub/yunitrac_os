#!/usr/bin/env python3
"""
Deterministic OTEL JSON ingest to Trace Contract v1.
Stdlib only. No payload text stored. Bounded stdout handled here.
"""
import argparse
import json
import sys
...
SEC = {
    "bad_args": "OTEL_BAD_ARGS",
    "io": "OTEL_IO_FAIL",
    "bad_json": "OTEL_BAD_JSON",
    "unsup": "OTEL_UNSUPPORTED_SPAN",
    "nonascii": "OTEL_NONASCII_FIELD",
    "policy": "OTEL_POLICY_MISSING",
    "internal": "OTEL_INTERNAL",
}
...
def fail(code: str):
    print(f"FAIL: otel_trace_ingest {SEC[code]}")
    sys.exit(1)
...
def extract_attributes(raw_attrs):
    # raw_attrs may be dict or list of key/value entries
    ...
    if isinstance(raw_attrs, list):
        ...
        for entry in raw_attrs:
            ...
            if isinstance(val, dict):
                # prefer stringValue; fallback to other simple types
...
def map_event_type(name: str, attrs: dict, status: str):
    lname = name.lower()
    if "tool" in lname or "gen_ai.tool.name" in attrs:
        return "TOOL_CALL"
    if "llm" in lname or "gen_ai.model" in attrs or "gen_ai.request.model" in attrs:
        return "LLM_CALL"
    if "hitl" in lname:
        return "HITL_REQUEST"
    if "final" in lname:
...
def main():
    ...
    if "artifacts" not in parts:
        fail("bad_args")
    idx_art = parts.index("artifacts")
    if idx_art + 1 >= len(parts) or parts[idx_art + 1] != "trace_otel":
        fail("bad_args")
    ...
    run_id_src = f"{input_sha}:otel_json_v0:{rules_version}".encode("utf-8")
    ...
    for sp in spans:
        if not isinstance(sp, dict):
            fail("bad_json")
        name = sp.get("name")
        span_id = sp.get("spanId") or sp.get("id") or ""
        if not isinstance(name, str) or not isinstance(span_id, str):
            fail("bad_json")
        span_short = span_id[:8]
        if not ascii_only(span_short):
            fail("nonascii")
        raw_attrs = sp.get("attributes")
        attrs = extract_attributes(raw_attrs)
        agent_id = attrs.get("agent.id", "agent0")
        if not isinstance(agent_id, str) or not ascii_only(agent_id):
            fail("nonascii")
        step = attrs.get("step", 0)
        if not isinstance(step, int) or step < 0:
            fail("bad_args")
        status = "FAIL" if sp.get("status", {}).get("code") == "STATUS_CODE_ERROR" else "OK"
        etype = map_event_type(name, attrs, status)

        payload_src = None
        for key in ("gen_ai.prompt", "gen_ai.request", "input", "output"):
            if key in attrs:
                payload_src = attrs[key]
                break
        payload_bytes = normalize_payload(payload_src)
    ...
    # Binary encoding (same as Trace Contract v1)
    ...
    (final_out / "ingest_status.json").write_text(
        json.dumps({"status": "PASS", "seccode": "OTEL_OK"}, sort_keys=True, separators=(",", ":")) + "\n",
        encoding="utf-8",
    )

    print("PASS: otel_trace_ingest")
